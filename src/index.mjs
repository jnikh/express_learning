...
app.get('/api/user/:id', (request, response) => {
  ...
  if (isNaN(parsId)) return response.status(400).send("Bad Request. User ID is invalid");
  const findUser = data.find((user) => user.id === parsId);
...
app.get('/api/followers/:name', (request, response) => {
  ...
  const findFollowers = list.find((follower) => follower.name.toLowerCase() === parsName);
...
app.put('/api/followers/:name',(request,response)=>{
  ...
  const findFollowerIndex= list.findIndex((follower)=>follower.name.toLowerCase() === parsName);
